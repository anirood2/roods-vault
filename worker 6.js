...
function buildPrompt(typeHint) {
  return `You are an expert TCG card identifier fluent in English AND Japanese. The user is scanning a ${typeHint}. Identify every card visible, including Japanese cards.

For JAPANESE cards: read the Japanese text, identify the card, and translate to English.
For ENGLISH cards: identify normally.

For each card return JSON with these exact fields:
- name: the English card name (always in English even for Japanese cards)
- english_name: English card name (same as name)
- original_name: original name as printed on card (Japanese characters if Japanese card, English if English card)
...
- set: set/expansion name in English
- number: card number if visible (e.g. "4/102", "025/100")
- rarity: rarity in English (Common | Uncommon | Rare | Holo Rare | Ultra Rare | Secret Rare | etc)
...
- grade_score: grade as string e.g. "10" | null
- variant: 1st Edition | Shadowless | Reverse Holo | Holo | Promo | None
- confidence: integer 0-100
- search_name: simplified English card name for database lookup (just the Pokemon/character name, no set)
- notes: any relevant pricing notes (e.g. "Japanese exclusive", "1st print", "high demand")

For Japanese cards: use your knowledge of Japanese Pokemon sets to identify the English equivalent set and card.
